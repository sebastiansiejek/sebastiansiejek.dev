...
import type { ProjectKey } from 'lib/portfolio/projects'
      ...
      <main id="main">
        <article>
          <header className="mx-auto w-full max-w-[1280px] px-5 pt-[clamp(4rem,9vw,8rem)] pb-[clamp(3rem,6vw,5rem)] max-md:px-4 max-md:pt-12">
            ...
            <PortfolioLabel className="mb-4">{project.kind}</PortfolioLabel>
          ...
          {project.image ? (
            ...
            >
              {['1', '2', '3', '5', '7', '13'].map((estimate) => (
                <PlanningPokerCard key={estimate}>{estimate}</PlanningPokerCard>
              ))}
            </div>
          )}
          ...
          <aside className="bg-portfolio-surface py-[clamp(5rem,10vw,8rem)]">
            <PortfolioContainer>
              ...
              <PortfolioButtonLink className="mt-8" href={`/${locale}#contact`}>
                {t('contact')}
              </PortfolioButtonLink>
            </PortfolioContainer>
          </aside>
